...
This repository provides replciation code for the methods outlined in the arcticle,"Performance of Health Care Service Area Definitions for Capturing Variation in Inpatient Care and Social Determinants of Health." Due to data restricted by DUAs and software licences in the original study, this code draws exclusively on publicly avaialble data to replicate the methods used to determine capture rates within a geographic health care service area (HCSA).
...
if (!file.exists(here("output/shapefiles/hsa-shapefile.rds"))) {
  ...
  sf_hsa %>% write_rds(here("output/shapefiles/hsa-shapefile.rds"))
}
sf_hsa <- read_rds(here("output/shapefiles/hsa-shapefile.rds"))
```
...
:::
![](media/geocorr-screenshot.png)

```{r}
census_tract_to_zip <- 
  read.csv(here("data/geocorr/geocorr2014_2332503261.csv"),skip=1) %>% 
  as_tibble() %>% 
  janitor::clean_names()
...
if (!file.exists(here("output/edge-lists/edge-list.rds"))) {
  # URL of the API
  api_url <-
        ...
        if (length(json_data) > 0) {
          if (nrow(json_data) < 1000) {
            ...
            repeat {
              ...
              if (nrow(json_data_) < (nrow(json_data) + 1000)) {
                df_ <- json_data_ %>%
                  filter(!grepl("\\*", TOTAL_DAYS_OF_CARE)) %>%
                  janitor::clean_names()
                break
  ...
  edge_list %>%
    bind_rows() %>%
    write_rds(here("output/edge-lists/edge-list.rds"))
  
} 

edge_list <- 
  read_rds(here("output/edge-lists/edge-list.rds"))
```
